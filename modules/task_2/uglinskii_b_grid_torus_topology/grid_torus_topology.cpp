...
bool IsExpectedGridTorus(const MPI_Comm grid_torus_comm,
                         const int exp_dims_count, const int* exp_dims_vals) {
  int topology_type = 0;
  ...
  if (real_dims_count != exp_dims_count)
  {
    ...
    MPI_Cart_get(grid_torus_comm, 2, topology_dims, topology_periods,
                 proc_coords);
