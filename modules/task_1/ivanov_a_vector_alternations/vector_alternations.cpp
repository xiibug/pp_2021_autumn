...
void fillVecWithRandValues(int* vec, int len)
{
	...
	for (int i = 0; i < len; i++)
		do vec[i] = (int)dist(rng) * (((int)dist(rng) & 0x01) ? -1 : 1);
	while (!vec[i]);
}
...
{ // parallel code
	...
	MPI_Scatterv((void*)vec, sendcounts, displs, MPI_INT, recvbuf, elemsForProc, MPI_INT, 0, MPI_COMM_WORLD);

	// counting alternations
	int localAlternations = sequentialCount_V2(recvbuf, elemsForProc);
	int globalAlterations = 0;
	MPI_Reduce((void*)&localAlternations, (void*)&globalAlterations, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
