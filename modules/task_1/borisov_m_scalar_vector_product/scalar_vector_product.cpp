...
int parallel_product(std::vector<int> vector_a, std::vector<int> vector_b) {
    ...
    MPI_Bcast(vector_a.data(), size, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(vector_b.data(), size, MPI_INT, 0, MPI_COMM_WORLD);

    for (int i = proc_rank * size / proc_count;
        i < (proc_rank + 1) * size / proc_count; i++) {
        local_result += vector_a[i] * vector_b[i];
    }
