...
{
    int my_rank, tasks, ccount = 0;    //variables for the rank of ongoing process, number of processes and symbol count respectively
...
double seqFrequency(char* symbol, std::string text)
{
    ...
    double freq = ((double)count / (double)N) * 100;
    return freq;
}
